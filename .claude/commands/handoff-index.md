Integrator-only refresh of the shared index `docs/progress.md`. Run this when shared/global state has changed (a slice merged, the harness flipped, a spec moved, new ADRs landed). For per-lane handoff use `/handoff <lane>` instead — this command never touches a lane file.
