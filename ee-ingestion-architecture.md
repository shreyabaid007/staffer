...
2. **Structured-only viability.** At current scale (eligible pools of single-digits to dozens after gates) exhaustive structured scoring without a vector recall stage is viable and more explainable; the vector path earns its keep once eligible-pool size × per-candidate LLM-score cost grows painful. Decide whether to ship the vector path now or defer it behind the index interface. **→ RESOLVED:** the vector path was built but deferred behind the index interface (AD-089), then **flipped ON by default** (AD-109, `index.recall.enabled: true`).
3. **Location overloading.** `Chennai-open`, `Location = "Remote (India)"`, and `remote_eligible` collapse three distinct concepts. Proposal: split into `home_location` / `onsite_open_cities` / `remote_within_country`. Needs a model decision. **→ RESOLVED by AD-086:** `Location.remote_eligible` was split into `remote_within_country: bool` + `onsite_cities: frozenset[str]`. The `remote_eligible` fields shown in the model snippets above (§3/§6) and in `CandidateIndexRecord` are superseded — code uses the AD-086 fields.
