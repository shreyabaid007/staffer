...
- **Eval:** Three-tier pytest harness (AD-095): **Tier 1** code-based invariant evaluators (deterministic, cassette LM, `make check`); **Tier 2** signature regression (`clarify`/`score` shape pinning, `make eval`); **Tier 3** live smoke + cassette drift guard (real LLM, key-gated, `make eval`). No LLM judge for objective invariants. **AI eval layer (AD-104/105/106):** hand-labelled golden set (20–40 cases, draft until human sign-off), DeepEval G-Eval narrative-faithfulness judge (validated against golden labels, adopted only if TPR/TNR ≥ 80%), deterministic Recall@K + contextual precision over retrieval. **Eval hardening (AD-XXX, c-010):** guardrail-detector validation (the c-009 jailbreak/bias/toxicity detectors scored as classifiers — precision/recall/F1/TPR/TNR + Cohen's κ + threshold-sweep calibration), a prompt-injection red-team (OWASP LLM01 — attack-success-rate = 0 through the guarded pipeline + a live susceptibility probe), and a counterfactual fairness eval (deterministic proxy-blindness + live sub-score parity). All non-gating (`make eval` only); the deterministic invariants remain the commit gate.
- **CLI:** Typer.
- **Web:** FastAPI + uvicorn (AD-113) — a thin JSON API + one self-contained static page (no SPA/build) over the **same** spine; `dsm/web/` is a composition root (peer of `dsm/cli/`) that reuses the CLI builders so the PII boundary is untouched (it never imports `dsm.pii` directly). `python-multipart` for demand-CSV upload.
- **Guardrails:** Guardrails AI (`guardrails-ai`, AD-XXX) — a **complementary** defence-in-depth input/output content-safety layer around the `PseudonymisedLM` call sites; it does **not** replace the PII boundary (AD-101/102), the deterministic gates (AD-002), or the citation check (AD-073). Hub validators: `detect_jailbreak` (input injection, local), `bespoke_minicheck` (narrative grounding, remote API), `bias_check` + `toxic_language` (narrative bias/toxicity, local); score-bounds is a deterministic Python clamp (not `valid_range`). `dsm/guardrails/` is a composition root wired at `dsm/cli/` — `dsm.match` never imports it (import-linter + AST tested). An **optional, eval-tier extra** (`staffer[guardrails]`): the guards import `guardrails` lazily and no-op when absent (each validator toggled under `config/default.yaml::guardrails`), so `make check` stays hermetic — only the deterministic score-bounds guard gates the commit; the ML/LLM validators (injection/grounding/bias/toxicity) live in `make eval`.
