...
- **CLI:** Typer.
- **Web:** FastAPI + uvicorn (AD-XXX) — a thin JSON API + one self-contained static page (no SPA/build) over the **same** spine; `dsm/web/` is a composition root (peer of `dsm/cli/`) that reuses the CLI builders so the PII boundary is untouched (it never imports `dsm.pii` directly). `python-multipart` for demand-CSV upload.
