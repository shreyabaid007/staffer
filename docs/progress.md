...
## Current status
- **Build phase:** **Real ingestion now reaches the gold layer** — landing + parse → bronze → normalized silver → **LLM-enriched, conflict-resolved canonical `GoldCandidate` per consultant** (PII-bracketed extraction with verified citations + provenance-weighted merge + snapshot reconcile/tombstones). The reasoning `PseudonymisedLM` is real-callable (OpenRouter), though its in-LM anonymise/deanonymise layer is still the Slice-0 pass-through stub; **index/retrieval (embeddings, Milvus) not yet built.** Contracts frozen with signed-off amendments — AD-075/076/077.
- **Active slice:** Most recent merges to `main` are Lane A ingestion enrich→gold (PR #12, `spec/a/003-ingest-enrich-gold`) + a follow-up fix so an idempotent re-run is a no-op for gold rather than tombstoning everything (PR #13, `fix/a/003-rerun-tombstone`). `dsm ingest` now runs enrich→merge→reconcile→gold and prints a PII-safe `── Gold ──` summary (AD-077/078/079 on top of AD-065…AD-076).
- **Harness (`make check`):** GREEN — format, lint, typecheck, 242 tests (6 skipped — opt-in real-data smoke), 3 import contracts all pass.
- **`main`:** Slice 0 foundation (PR #3) + per-lane progress files (PR #4); index refresh (PR #6); real gates/rank/no-match (PR #5, `feat/c/001-gates-rank`); ingestion architecture docs + AD-065…AD-074 (PR #9); ingestion landing+parse → bronze (PR #10, `feat/a/001-ingest-landing-parse`); ingestion silver + identity vault seed (PR #11, `feat/a/002-ingest-silver-identity`); ingestion enrich → canonical gold (PR #12, `spec/a/003-ingest-enrich-gold`) + re-run tombstone fix (PR #13).
...
- `make check` — 242 tests green (6 skipped — opt-in real-data smoke), 0 type errors, 3 import contracts (gates ⊥ PII/index/LLM; no direct LLM provider access; ingest ⊥ match/index — ingest may now use the `dsm.pii` boundary + `dspy`, the sanctioned LLM path, AD-078). Importable ROLE-01/02/03 seed fixtures in `tests/fixtures/` (reusable by `dsm/eval/`).
...
- _Signed-off design (not a spec):_ [`ee-ingestion-architecture.md`](../ee-ingestion-architecture.md) — full ingestion subsystem, accepted as AD-065…AD-074. a-001/a-002/a-003 implement its landing+parse, silver, and enrich+gold stages; the index/embedding stage is still to be spec'd.
- _Next (planned):_ Lane A follow-on ingestion spec for the index/embedding stage (capability-only embed + `skill_set`, gated by `gold_hash`; Milvus + Modal BGE, AD-072/074); Lane C `c-002-*` for the real `PseudonymisedLM` provider/anonymiser, NER + client-org-dictionary hardening, and the encrypted identity-vault write path. Neither written yet.

## Decisions
- Authoritative log: `docs/decision.md` (current range AD-001 … AD-079; next starts at AD-080). Recently landed (a-003): **AD-077 — relax frozen `EvidenceCitation`** with optional `source_hash`/`locator`; `text` is the verified verbatim quote (one citation type system-wide); **AD-078 — relax the ingest import boundary to the `dsm.pii` layer + `dspy`** (the sanctioned LLM path; `match`/`index`/`modal`/`httpx` stay forbidden), adding generic `redact.py`/`leakscan.py` (Lane A seeds, Lane C hardens) and superseding AD-076's vault-only narrowing — import contracts 4→3; **AD-079 — feedback *facts* in gold, feedback *score* deferred to match/score** (Lane B), superseding ee-ingestion §6's `performance_feedback_score` gold field. Prior: **AD-075/076** (silver/identity) and **AD-065…AD-074** (ingestion architecture).
