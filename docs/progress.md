...
- **Active slice:** Most recent merge to `main` is Modal embedder + reranker deployment (PR #14, `feat/a/004-modal-embed-rerank`). Prior: ingestion enrich→gold (PR #12) + tombstone fix (PR #13). **c-001 gates+rank (PR #5) is deprecated (AD-085)** — gating/ranking/scoring design is provisional pending the query-time pipeline architecture.
...
- **`main`:** Slice 0 foundation (PR #3) + per-lane progress files (PR #4); index refresh (PR #6); gates/rank/no-match (PR #5, `feat/c/001-gates-rank` — **deprecated AD-085**); ingestion architecture docs + AD-065…AD-074 (PR #9); ingestion landing+parse → bronze (PR #10, `feat/a/001-ingest-landing-parse`); ingestion silver + identity vault seed (PR #11, `feat/a/002-ingest-silver-identity`); ingestion enrich → canonical gold (PR #12, `spec/a/003-ingest-enrich-gold`) + re-run tombstone fix (PR #13); **Modal embedder + reranker deployment (PR #14, `feat/a/004-modal-embed-rerank`)**.

## Works end-to-end right now
- `uv run dsm match --role-id ROLE-STUB-01` — runs the pipeline over stub ingest and prints a `ShortlistResult` / `NoMatchResult` JSON. **Note:** the gates+rank modules are deprecated (AD-085); the query-time pipeline design will replace this flow.
...
- **Reusable PII layer** ([`dsm/pii/`](../dsm/pii/)) — generic `redact.py` (deterministic known-PII strip first + Presidio NER seam + de-anonymize), `leakscan.py` (outbound hard gate, fails the build on residual PII), and the `vault.py` `candidate_id = HMAC(email)` derivation; ingest reaches the LLM only through `PseudonymisedLM` (AD-078). Encrypted at-rest vault + NER/org-dictionary hardening are deferred to a later phase.
- **~~Real deterministic gates~~** ([`dsm/match/gates.py`](../dsm/match/gates.py)) — **DEPRECATED (AD-085).** Gating design is provisional; will be revisited in the query-time pipeline architecture.
- **~~Real ranking~~** ([`dsm/match/rank.py`](../dsm/match/rank.py)) — **DEPRECATED (AD-085).** Ranking design is provisional; will be revisited in the query-time pipeline architecture.
- **~~No-match path + near-misses~~** ([`dsm/cli/commands.py`](../dsm/cli/commands.py)) — **DEPRECATED (AD-085).** Near-miss assembly is provisional; will be revisited in the query-time pipeline architecture.
...
- `specs/c-001-gates-rank/` — ~~complete, approved, merged to `main` (PR #5)~~ **DEPRECATED (AD-085).** Gates+rank implementation is provisional; gating/ranking/scoring design will be revisited in the query-time pipeline architecture.
...
- _Next (planned):_ **Query-time pipeline architecture** (companion to `ee-ingestion-architecture.md`) — will settle gating, retrieval, scoring, and ranking design. Follow-on: real `PseudonymisedLM` provider/anonymiser, NER + client-org-dictionary hardening, encrypted identity-vault write path.
